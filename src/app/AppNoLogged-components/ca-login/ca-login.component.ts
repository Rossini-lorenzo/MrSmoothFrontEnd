...
resp.subscribe(data=>{console.log("DATA LOGIN ",data);this.accessApi(JSON.parse(data))});
  }
...
resp.subscribe(data=>{
      this.response=data;
       if (payload.role=="ROLE_PROMOTE" ){this.router.navigate(['/home']);console.log("promote");}
       else {this.router.navigate(['/home']);console.log("user");}

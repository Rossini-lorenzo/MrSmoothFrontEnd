...
export class AroundPromoteService {
  ...
  modifyPlace(nomeLuogo: string, descrizione: string, posti: string,idPlace : string): Observable<HttpResponse<Object[]>> {
    let tokenStr = 'Bearer ' + localStorage.getItem('token');

    const headers = new HttpHeaders().append('Authorization', tokenStr);
    console.log(idPlace);
    return this.httpClient.post<any>("http://localhost:8080/promote/modifyPlace?&nomeLuogo=" + nomeLuogo + "&descrizione=" + descrizione + "&posti=" + posti+ "&idPlace=" + idPlace, null,
